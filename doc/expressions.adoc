...
presented as opaque values of the `view` type, and created and
manipulated operated on using functions of the `View` module.  Values
of view type can be stored in and retrieved from data structures, and
synthesized attributes.  A value of `view` type can be considered to
...
Their most important use is in the grammar sub-language, which
...
Although the all values of other primitive types in the expression
sub-language are stateless, values of `view` type are stateful.  When
...
Although values of `view` type are opaque, they can be compared for
equality and inequality using the generic `=` and `!=` operators.

Different values of view type have independent state.  View values
...
error returned to the application using the engine.

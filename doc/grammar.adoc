...
....
  NT n (v: view, i: int, j: int) {n: unit} := ... ;;
...
in the production rule for `T`, its `v` and `i` attributes are
provided values at the same index in the `vs` and `is` lists
respectively.

NOTE: An internal constraint is generated to ensure that any lists

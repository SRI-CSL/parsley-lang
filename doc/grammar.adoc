...
Bits upto a specified byte-boundary `n` can be skipped over using the
alignment specifier `$align<n>`, where `n` is required to be a
...
bits to end at a cursor position 64 bits from the starting point.  The
`n` in an alignment specifier `align<n>` is required to be a multiple
of 8.

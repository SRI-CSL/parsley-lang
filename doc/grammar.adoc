...
strings* are a common case of regular expression, and can be directly
...
attributes, their values need to specified within a pair of `<` and
...
elements in its scope will not be executed if evaluates to false.

*Actions* are blocks, demarcated within `{` and `}` markers, within
which some computation is done.  This computation typically does one
of more of: assign values to the synthesized attributes of the
...
rule elements comprising the choice.  This value matched by this
...
usual unbounded repeat, bounded repeat, option, choice and sequence
combinators.  These combinators are denoted by the same symbol used by
the corresponding rule element combinator. As usual, `(` and `)` can
be used for grouping purposes.

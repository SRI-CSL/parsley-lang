...
==== Checking Parsley Files

Much of this tutorial focuses on how to create Parsley files.
But the capability to perform a check on a Parsley file to ensure its validity is also critical.
Indeed, the Parsley file checker notifies the user when errors, such as type check errors, occur.
Most errors belong to one of three error classes: syntax errors, type check errors, and pattern matching errors.
Additionally, there is a catch-all fourth error class for other errors.
...
The first two classes of errors and the last class should be clear enough.
Understanding the third class of errors, pattern matching errors, requires knowledge of case expressions, which we have yet to cover.

In addition to the errors above, we note that empty or relatively simple Parsley files are perfectly acceptable.

Some examples of erroneous Parsley files are provided in `./doc/tutorial/examples/erroneous_examples/`.

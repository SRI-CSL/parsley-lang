...
let make_tvar (id: ident) : type_expr =
  {type_expr = TE_tvar id;
   type_expr_loc = Location.loc id}

let make_tname (name: string) loc : type_expr =
  {type_expr = TE_tname (Location.mk_loc_val name loc);
   type_expr_loc = loc}

let make_tname_id (id: ident) : type_expr =
  let loc = Location.loc id in
  {type_expr = TE_tname id;
   type_expr_loc = loc}

let make_type_app (name: string) (args: type_expr list) loc
    : type_expr =
  let c = make_tname name loc in
  {type_expr = TE_tapp (c, args);
   type_expr_loc = loc}

let make_type_app_id (id: ident) (args: type_expr list) loc
    : type_expr =
  let c = make_tname_id id in
  {type_expr = TE_tapp (c, args);
   type_expr_loc = loc}
...
let make_bitvector_type (n: int) loc : type_expr =
  assert (n > 0);
  let n = make_tname (string_of_int n) loc in
  make_type_app "bitvector" [n] loc

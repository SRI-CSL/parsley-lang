...
let environment_as_list e =
  let rec conv acu = function
    | EEmpty                   -> acu
    | EEnvFrame (env, name, v) -> conv ((name, v)::acu) env in
  conv [] e
...
let generalize old_pool young_pool =
  ...
  let sorted       = Array.make (young_number + 1) [] in
  let young        = Mark.fresh () in
  ...
  for k = 0 to young_number do
    let rec traverse v =
      ...
      (* If the variable isn't marked ``young'' or ``visited'', then it must
         ...
         any further. *)
      else if not (Mark.same desc.mark visited)
      then begin
          desc.mark <- visited;
          if   k < desc.rank
          then desc.rank <- k
  ...
  for k = 0 to young_number - 1 do
    try
      List.iter (fun v ->
          if   not (UnionFind.redundant v)
          then register old_pool v
        ) sorted.(k)
    with Invalid_argument _ ->
      (* The invariant is broken. *)
  ...
  List.iter (fun v ->
      if   not (UnionFind.redundant v)
      then let desc = UnionFind.find v in
           if   desc.rank < young_number
           then register old_pool v
           else (desc.rank <- IntRank.none;
                 if   desc.kind = Flexible
                 then desc.kind <- Rigid)
    ) sorted.(young_number)
  ...
  try
    List.iter (fun v ->
        ...
        match desc.structure with
          ...
          | _ ->
              if   Mark.same desc.mark m
              then raise (DuplicatedMark m);
              desc.mark <- m
      ) vl
...
let generic_variables pos vl =
  List.iter (fun v ->
      let desc = UnionFind.find v in
      if   IntRank.compare desc.rank IntRank.none <> 0
      then raise (Error (pos, CannotGeneralizeRank (v, desc.rank)))
    ) vl

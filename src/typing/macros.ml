...
(* In addition to expression language processing, this function also
   handles some processing required by the grammar language:
   currently, it inserts list-length constraints generated by map-view
   multi-assignments.  Since this results in two rule-elements being
   generated (the constraint followed by the map-view) from a single
   rule-element (the map-view), this function returns a list of
  ...
  match re.rule_elem with
    ...
    | RE_named (n, re') ->
        ...
        (* We expect to see at most two elements from processing re'.
           Since the first is the constraint, the named binder is
           placed on the second.  This helps avoid having the
           constraint affecting the type of `n`.  However, it will
           still be affected by more deeply nested constraints.*)
        let res =
          match re'' with
            | [re'] ->
                [{re with rule_elem = RE_named (n, re')}]
            | c :: [re'] ->
                c :: [{re with rule_elem = RE_named (n, re')}]
            | _ ->
                assert false in
        ctx, res
    | RE_action a ->
    ...
    | RE_choice res ->
        (* If any element of the choice expands to multiple elements,
           it needs to be converted into a sequence. *)
        let ctx, res' =
          List.fold_left (fun (ctx, res') re ->
              let ctx, res = expand_rule_elem ctx re in
              let re = match res with
                  | []    -> assert false
                  | [re'] -> re'
                  | _     -> {re with rule_elem = RE_seq res} in
              ctx, re :: res'
            ) (ctx, []) res in
        ctx, [{re with rule_elem = RE_choice (List.rev res')}]
    | RE_star (re', oe) ->
        let ctx, re'' = expand_rule_elem ctx re' in
        let ctx, oe   = match oe with
            | None   -> ctx, None
...
and mk_list_constraint re_mod e args loc : (unit, unit, mod_qual) rule_elem =
  ...
  let vl  = mk_expr (E_apply (ll, [e]))   e.expr_loc in
  let als =
    List.fold_left (fun als (_, a, ae) ->
        match a with
          | A_eq ->
              als
          | A_in ->
              let m  = Location.mk_loc_val "List"    ae.expr_loc in
              let f  = Location.mk_loc_val "length"  ae.expr_loc in
              let ll = mk_expr (E_mod_member (m, f)) ae.expr_loc in
              let al = mk_expr (E_apply (ll, [ae]))  ae.expr_loc in
              al :: als
      ) [] args in
  let cs =
    List.fold_left (fun cs al ->
        let c = mk_expr (E_binop (Eq, vl, al)) loc in
        c :: cs
      ) [] als in
  let ce =
    match List.rev cs with
      | [] ->
          assert false
      | hd :: tl ->
          List.fold_left (fun ce c ->
              mk_expr (E_binop (Land, c, ce)) loc
            ) hd tl in

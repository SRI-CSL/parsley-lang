...
let lookup_field lc (l: Smt_cfg.SMTNode.node) (f: string) : Smt_cfg.SMTNode.node option =
  match l with
    | N_record fs ->
        List.assoc_opt f fs
    | _ ->
        internal_error lc (Type_error ("." ^ f, 1, T_unit, T_record [f, T_empty]))
...
let set_field lc (l: Smt_cfg.SMTNode.node) (f: string) (v: Smt_cfg.SMTNode.node) : Smt_cfg.SMTNode.node =
  match l with
    | N_record fs ->
...
(* subterm extraction *)
let subterm lc (v: Smt_cfg.SMTValue.value) (o: Anfcfg.Anf.occurrence) : Smt_cfg.SMTValue.value =
  let rec walk (v: Smt_cfg.SMTValue.value) so =
    match v, so with
      | _, [] ->
          v

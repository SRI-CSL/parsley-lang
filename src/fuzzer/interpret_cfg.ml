...
type fuzzing_result = (value, state) result * state

let rec do_jump lc (s: state) (l: Cfg.label) : fuzzing_result =
  assert (Cfg.is_static l);
  let b = get_block lc s l in
  do_closed_block s b

and do_return _lc (s': state) (l: Cfg.label) : fuzzing_result =
...
and do_exit_node (s: state) (n: Cfg.Node.exit_node) : fuzzing_result =
  match n with
    ...
    | Cfg.Node.N_check_bits (loc, (mbb, pat), lsc, lf) ->
        print_endline "callign check bits";
        let bits, s = collect_bits loc s in
        if   not (match_bits_bound bits mbb)
        then let m   = Ir_printer.string_of_mbb mbb in
             let err = Internal_errors.Bitsbound_check m in
    ...
    | Cfg.Node.N_jump (loc, l) ->
        print_endline "callign jump";
        do_jump loc s l
    | Cfg.Node.N_return (loc, l) ->
        print_endline "callign return";
        do_return loc s l
    | Cfg.Node.N_constraint (loc, v, lsc, lf) ->
        print_endline "callign constrain";
...
and do_closed_block (s: state) (b: Cfg.closed) : fuzzing_result =

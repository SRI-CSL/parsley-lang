...
let mk_scf ckopts init_envs tenv (spec: TypedAst.spec_module) : Scf.spec_scf =
  try  Scf_of_spec.lower_spec ckopts.co_trace_cfg_build
  ...
  with
    ...
    | Scf.Error (l, e) ->
        Errors.handle_exception (Printexc.get_backtrace ()) l (Scf.error_msg e)
    | Scf_validator.Error (l, e) ->
        Errors.handle_exception (Printexc.get_backtrace ()) l (Scf_validator.error_msg e)
    | Scf_zipper.Error (l, e) ->
        Errors.handle_exception (Printexc.get_backtrace ()) l (Scf_zipper.error_msg e)

let scf_of_ast _verbose ckopts ast : Scf.spec_scf =
  let init_envs, tenv, tspec = type_check ckopts ast in
  let cfg = mk_scf ckopts init_envs tenv tspec in
  if   ckopts.co_show_cfg
  then Scf_printer.print_spec cfg;
  cfg

let scf_of_spec verbose ckopts sopts spec_file : Scf.spec_scf =
  let ast = parse_spec ckopts sopts spec_file in
  scf_of_ast verbose ckopts ast

let check_spec verbose ckopts sopts spec_file : unit =
  ignore (scf_of_spec verbose ckopts sopts spec_file)
